...
export const UploadFile = ({
    ...
    const onChange = (e: any) => {
        ...
        Promise.all(filesArr.map((file: any) => toBase64(file)))
            .then(recentFiles => {
    ...
    const removeFile = (f: any) => {
        if (isArray) setFieldValue(name, field.value.filter((x: any) => x !== f));
    ...
    return (
        <div className={mode === 'horizontal' ? 'row' : ''}>
            ...
            <div className={GetClassName(labelWidth, false)}>
                <div style={styles}>
                    <input style={{display: 'none'}} ref={fileRef} type="file" multiple={maxNumber > 1}
                           onChange={onChange}/>
                    {!_disabled && !(innerValue.length >= maxNumber) && (
                        <button
                            ...
                            <svg width="462" height="181" viewBox="0 0 462 181" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    {innerValue.map((x: any) =>
                        <div className="file-preview"><Format input={x}/><span onClick={() => removeFile(x)}> x</span>
                        </div>
                    )}
